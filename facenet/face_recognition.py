...
model = "FaceRecognition_MTCNNandFaceNet/facenet/model/20170512-110547.pb"
...
def main(test_path,top_n = 100):
    ...
    f=open('FaceRecognition_MTCNNandFaceNet/facenet/model/information.txt','r')
    ...
    while(function_choose!=function_exit):   
            ...
            if function_choose==function_collection:
                ...
                f1=open('FaceRecognition_MTCNNandFaceNet/facenet/model/information.txt','w')
                ...
                while(i!=7):
                    ...
                    if k & 0xFF == ord('q'):           
                        i=i+1
                        cv2.imwrite("FaceRecognition_MTCNNandFaceNet/facenet/pics/anchors/"+str(identity_key)+str(i)+".jpg", frame)
                        if i==3:
                ...
                embedDatabaseAndSaveAsNpy()    
                        ...
                        while(1):
                            ...
                            if cv2.waitKey(1) & 0xFF == ord('w'):
                                cv2.imwrite("FaceRecognition_MTCNNandFaceNet/facenet/imgs/123.jpg", frame)
                                break
                        cap.release() 
                        cv2.destroyAllWindows()
                        ...
                        if os.path.exists(database_file) == False:
                            ...
                            data=np.load('FaceRecognition_MTCNNandFaceNet/facenet/model/database.npy')
                            Data=np.array(data)
                            hang=Data.shape[0]
                            Data_revised=np.delete(Data,hang-1,axis=0)
                            np.save('FaceRecognition_MTCNNandFaceNet/facenet/model/database.npy',Data_revised)
                            os.remove('FaceRecognition_MTCNNandFaceNet/facenet/imgs/123.jpg')
                            os.remove('FaceRecognition_MTCNNandFaceNet/facenet/pics/anchors/123.jpg')
                            os.remove('FaceRecognition_MTCNNandFaceNet/facenet/pics/resize_anchors/123.jpg')
                            return output
